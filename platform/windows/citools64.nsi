...
#META
###################
!define APP_NAME "CiTools"
!define COMP_NAME "Tox"
!define WEB_SITE "https://github.com/TokTok/ci-tools"
!define VERSION "1.0.0.0"
!define DESCRIPTION "CiTools Installer"
!define COPYRIGHT "The Tox Project"
!define INSTALLER_NAME "setup-citools.exe"
!define MAIN_APP_EXE "bin\citools.exe"
!define REG_ROOT "HKLM"
!define REG_APP_PATH "Software\Microsoft\Windows\CurrentVersion\App Paths\citools.exe"
...
!ifdef REG_START_MENU
  ...
  !define MUI_STARTMENUPAGE_DEFAULTFOLDER "CiTools"
...
${CreateShortcut} "$DESKTOP\CiTools.lnk" "$INSTDIR\${MAIN_APP_EXE}" "" "" ""
FunctionEnd
...
!define MUI_FINISHPAGE_RUN_FUNCTION Launch_CiTools_without_Admin
!define MUI_FINISHPAGE_RUN
!define MUI_FINISHPAGE_LINK "Find CiTools on GitHub"
!define MUI_FINISHPAGE_LINK_LOCATION "https://github.com/TokTok/ci-tools"
!insertmacro MUI_PAGE_FINISH

Function Launch_CiTools_without_Admin
   SetOutPath $INSTDIR
...
###################
  ...
  Section "Create install directory"
    ...
    FileClose $0
    FileOpen $0 "$TEMP\CiTools-install-file-permissions.txt" w
      FileWriteUTF16LE  $0 "$INSTDIR"
      FileWriteUTF16LE  $0 "$\r$\n"
      DetailPrint "Writing to file: $2"
      FileWriteUTF16LE  $0 "$2"
    FileClose $0
    nsExec::Exec 'icacls "" /restore "$TEMP\CiTools-install-file-permissions.txt"'
  SectionEnd
...
Section "Install"
  ...
  File /nonfatal "citools\platforms\*.*"
  ${SetOutPath} "$INSTDIR\bin"

  ${CreateDirectory} "$INSTDIR\bin\tls"
  ${SetOutPath} "$INSTDIR\bin\tls"
  File /nonfatal "citools\tls\*.*"
  ${SetOutPath} "$INSTDIR\bin"

  # Create shortcuts
  ${CreateDirectory} "$SMPROGRAMS\CiTools"
  ${CreateShortCut} "$SMPROGRAMS\CiTools\CiTools.lnk" "$INSTDIR\${MAIN_APP_EXE}" "" "" ""
  ${CreateShortCut} "$SMPROGRAMS\CiTools\Uninstall CiTools.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Write setup/app info into the registry
  SetRegView 64
  ${WriteRegStr} "${REG_ROOT}" "${REG_APP_PATH}" "" "$INSTDIR\${MAIN_APP_EXE}"
  ${WriteRegStr} "${REG_ROOT}" "${REG_APP_PATH}" "Path" "$INSTDIR\bin\"
  ${WriteRegStr} ${REG_ROOT} "${UNINSTALL_PATH}" "DisplayName" "CiTools"
  ...
  ${WriteRegStr} ${REG_ROOT} "${UNINSTALL_PATH}" "URLInfoAbout" "https://toktok.github.io/ci-tools"
SectionEnd
...
Section Uninstall
  ...
  RMDir /r /REBOOTOK "$SMPROGRAMS\CiTools"

  ;Remove registry keys
  SetRegView 64
  DeleteRegKey ${REG_ROOT} "${REG_APP_PATH}"
  DeleteRegKey ${REG_ROOT} "${UNINSTALL_PATH}"
  DeleteRegKey HKCR "Applications\citools.exe"
SectionEnd
